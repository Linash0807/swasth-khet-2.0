...
const DynamicPricingService = require('../services/pricingService');
...
// @desc    Get all marketplace listings with price analytics
...
router.get('/', async (req, res) => {
  try {
    ...
    // Add price analytics to listings
    const enrichedListings = listings.map(listing => ({
      ...listing.toObject(),
      priceAnalytics: {
        marketPrice: DynamicPricingService.getMarketPrice(listing.crop),
        priceAlert: DynamicPricingService.generatePriceAlert(
          listing.crop,
          listing.price,
          DynamicPricingService.getMarketPrice(listing.crop) || listing.price
        )
      }
    }));
...
// @desc    Get price suggestion for a crop
// @route   GET /api/marketplace/price-suggestion
// @access  Private
router.get('/price-suggestion', async (req, res) => {
  try {
    const { crop, quantity, quality = 'standard', organic = false } = req.query;
    ...
    const priceRange = DynamicPricingService.getSuggestedPriceRange(
      crop,
    ...
    if (!priceRange) {
      return res.json({
        success: true,
        data: {
          warning: 'No market data available for this crop',
          suggestion: 'Please research local market prices'
        }
      });
    }
...
// @desc    Get market price comparison
// @route   GET /api/marketplace/price-comparison/:crop
// @access  Private
router.get('/price-comparison/:crop', async (req, res) => {
  try {
    ...
    const listings = await Marketplace.find({
      ...
      status: 'active'
    }).select('crop price quantity quality organic');

    const comparison = DynamicPricingService.getPriceComparison(listings);
    const marketPrice = DynamicPricingService.getMarketPrice(crop);
    ...
    res.json({
      ...
      data: {
        crop,
        marketPrice,
        comparison: comparison[crop.toLowerCase()],
